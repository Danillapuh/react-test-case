...
export const ChangeItemForm: React.FC<{item: Item}> = ({item})=>{
    ...
    const onSubmit = (data:any)=>{
        ...
        editItem({...data, id: item.id}).unwrap().then(res=>{
            /* formRef.current?.reset() */
            alert('изменен')
        })
    }
    return <Form ref={formRef} onSubmit={onSubmit} className="max-w-sm mx-auto p-3 flex flex-col h-full">
    ...
    <div className="flex justify-center gap-3">
        <button className="px-6 py-2 w-full text-white bg-neutral-600 hover:bg-neutral-700 rounded-md">Отмена</button>
